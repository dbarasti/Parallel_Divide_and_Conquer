...
static int *generateRandomArray(int n) {
    ...
    std::cout << "end generateRandomArray" << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    if (nwork == 1)
        // sequential version
        res = seqDC(op, cond, seq, divide, mergeMS);
    else {
        res = parDC(op, cond, seq, divide, mergeMS, checkCutOff);
    }
